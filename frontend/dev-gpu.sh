...
if [ "$OS" == "macos" ]; then
    echo ""
    echo -e "${BLUE}🍎 Building apple-speech-helper sidecar (debug)...${NC}"

    APPLE_SPEECH_HELPER_DIR="$WORKSPACE_ROOT/apple-speech-helper"

    if command_exists swift; then
        (cd "$APPLE_SPEECH_HELPER_DIR" && swift build)
        ...
        APPLE_SPEECH_SRC="$APPLE_SPEECH_HELPER_DIR/.build/debug/apple-speech-helper"
