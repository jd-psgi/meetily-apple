    ...
    tokio::spawn(async move {
        ...
        for worker_id in 0..NUM_WORKERS {
            let engine_clone = match &transcription_engine {
                ...
                #[cfg(target_os = "macos")]
                TranscriptionEngine::AppleSpeech(e) => TranscriptionEngine::AppleSpeech(e.clone()),
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    let confidence_threshold = match &engine_clone {
                                        ...
                                        #[cfg(target_os = "macos")]
                                        TranscriptionEngine::AppleSpeech(_) => 0.0, // SpeechAnalyzer has no confidence, accept all
                                    };
...
async fn transcribe_chunk_with_provider<R: Runtime>(
    ...
    match engine {
        ...
        #[cfg(target_os = "macos")]
        TranscriptionEngine::AppleSpeech(apple_speech_engine) => {
            match apple_speech_engine.transcribe_audio(speech_samples).await {
                Ok(text) => {
                    let cleaned_text = text.trim().to_string();
                    if cleaned_text.is_empty() {
                        return Ok((String::new(), None, false));
                    ...
                    info!(
                        "Apple Speech transcription complete for chunk {}: '{}'",
                        chunk.chunk_id, cleaned_text
                    );

                    // SpeechAnalyzer doesn't provide confidence or partial results
                    Ok((cleaned_text, None, false))
                }
                Err(e) => {
                    error!(
                        "Apple Speech transcription failed for chunk {}: {}",
                        chunk.chunk_id, e
                    );
